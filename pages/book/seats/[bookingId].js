...
export default function SeatSelectionPage() {
  ...
  const toggleSeat = (label) => {
    ...
    setSelected((prev) => {
      ...
      if (prev.length >= data.seats_needed) return prev; // already picked enough
      return [...prev, label];
    });
  };
  ...
  const handleSave = async () => {
    ...
    if (res.ok) {
      ...
      setMessage('Seats and meal preference saved!');
      setSaved(true);
    } else {
      setMessageType('danger');
  ...
  return (
    <div className="container mt-4">
      ...
      <p className="text-muted">
        {data.flight_number} · {data.passenger_name} · choose {data.seats_needed} seat
        {data.seats_needed > 1 ? 's' : ''}
      </p>
      {message && <div className={`alert alert-${messageType}`}>{message}</div>}
      ...
      <div className="d-flex gap-4 flex-wrap">
            ...
            {seatLabels.map((label) => {
              ...
              return (
                <button
                  ...
                  style={col === 'D' ? { marginLeft: '18px' } : undefined}
                  title={label}
                >
                  {label}
                </button>
        ...
        <div style={{ minWidth: '260px' }}>
          ...
          <p>
            Selected: <strong>{selected.length ? selected.join(', ') : 'none yet'}</strong> (
            {selected.length}/{data.seats_needed})
          </p>

          <button
            className="btn btn-primary w-100"
            disabled={selected.length !== data.seats_needed}
            onClick={handleSave}
          >
            Save Selection
          </button>
      ...
      <style jsx>{`
        ...
        span .seat-legend.seat-available {
          background: #eef2f7;
          border: 2px solid #c7d2e0;
        }
        span .seat-legend.seat-selected {
          background: #0d6efd;
        }
        span .seat-legend.seat-taken {
          background: #e2e2e2;
        }
      `}</style>
    </div>
