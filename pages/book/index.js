...
export default function BookFlightPage() {
  ...
  return (
    <div className="container mt-4">
      ...
      {step === 'search' && (
        <form onSubmit={handleSearch} className="row g-2">
          <div className="col-md-3">
            <input className="form-control" placeholder="From" value={searchForm.origin} onChange={(e) => setSearchForm({ ...searchForm, origin: e.target.value })} />
          </div>
          <div className="col-md-3">
            <input className="form-control" placeholder="To" value={searchForm.destination} onChange={(e) => setSearchForm({ ...searchForm, destination: e.target.value })} />
          </div>
          <div className="col-md-3">
            <input className="form-control" type="date" value={searchForm.date} onChange={(e) => setSearchForm({ ...searchForm, date: e.target.value })} />
          </div>
          <div className="col-md-3">
            <button className="btn btn-primary w-100" type="submit">Search Flights</button>
          </div>
        </form>
      )}
      ...
      {step === 'results' && (
        <div className="mt-4">
          <button className="btn btn-link mb-2" onClick={() => setStep('search')}>← New Search</button>
          ...
          {results.map((f) => (
            <div key={f.id} className="card mb-2 p-3 d-flex flex-row justify-content-between align-items-center">
              <div>
                ...
                <br />
                {new Date(f.departure_time).toLocaleString()} · {f.available_seats} seats left · ${f.dynamic_price ?? f.price}
              </div>
              <button className="btn btn-success" onClick={() => selectFlight(f)}>Select</button>
            </div>
          ))}
        </div>
      )}
      ...
      {step === 'details' && selectedFlight && (
        <form onSubmit={handleBook} className="mt-4">
          <button type="button" className="btn btn-link mb-2" onClick={() => setStep('results')}>← Back to results</button>
