...
export default function AdminFaultsPage() {
  const [faults, setFaults] = useState([]); const [message, setMessage] = useState(''); const [messageType, setMessageType] = useState('info');
  ...
  const handleStatusChange = async (id, status) => {
    ...
    setMessageType(res.ok ? 'success' : 'danger'); setMessage(status === 'Resolved' ? 'Fault resolved — aircraft is Available again.' : data.message || data.error); loadFaults();
  };
  return (
    <div className="container mt-4">
      ...
      <table className="table table-striped align-middle">
        ...
        <tbody>
          {faults.map((f) => (<tr key={f.id}><td>{f.tail_number} ({f.model})</td><td className="small">{f.fault_description}</td><td><span className={`badge ${SEVERITY_BADGE[f.severity]}`}>{f.severity}</span></td><td>{f.reported_by_name || 'Unknown'}</td><td>{new Date(f.reported_at).toLocaleString()}</td><td><select className="form-select form-select-sm" style={{ width: 'auto' }} value={f.status} onChange={(e) => handleStatusChange(f.id, e.target.value)}>{STATUSES.map((s) => (<option key={s} value={s}>{s}</option>))}</select><span className={`badge ms-2 ${STATUS_BADGE[f.status]}`}>{f.status}</span></td></tr>))}
          {faults.length === 0 && <tr><td colSpan={6} className="text-muted">No fault reports yet.</td></tr>}
        </tbody>
      </table>
    </div>
  );
