...
export default function AdminGatesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      setMessage('Gate added.');
      setForm({ gate_number: '', terminal: '' });
      loadGates();
    } else {
        ...
        <tbody>
          {gates.map((g) => (
            <tr key={g.id}>
              ...
              <td>{g.terminal}</td>
              <td>
                <span className={`badge ${STATUS_BADGE[g.status] || 'bg-secondary'}`}>{g.status}</span>
              </td>
              <td>
                <button className="btn btn-sm btn-danger" onClick={() => handleDelete(g.id)}>
                  Remove
                </button>
              </td>
            </tr>
