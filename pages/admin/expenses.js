...
export default function AdminExpensesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      setForm({ category: 'Fuel', description: '', amount: '', expense_date: '' });
      loadExpenses();
    } else {
    ...
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit} className="border rounded p-3 mb-4">
        <div className="row g-2">
          ...
          <div className="col-md-2">
            <input className="form-control" type="number" step="0.01" placeholder="Amount ($)" required value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
          </div>
          <div className="col-md-3">
      ...
      <table className="table table-striped align-middle">
        ...
        <tbody>
          ...
          {expenses.length === 0 && (<tr><td colSpan={4} className="text-muted">No expenses logged yet.</td></tr>)}
        </tbody>
      </table>
    </div>
  );
