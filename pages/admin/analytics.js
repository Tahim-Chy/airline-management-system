  ...
  useEffect(() => {
    ...
    monthlyChartRef.current = new Chart(monthlyCanvasRef.current, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Revenue', data: data.monthly.map((m) => m.revenue), backgroundColor: '#0d6efd' },
          { label: 'Expenses', data: data.monthly.map((m) => m.expenses), backgroundColor: '#dc3545' },
        ],
      },
    ...
    flightChartRef.current = new Chart(flightCanvasRef.current, {
      type: 'bar',
      data: {
        labels: data.revenue_by_flight.map((f) => f.flight_number),
        datasets: [{ label: 'Revenue by Flight', data: data.revenue_by_flight.map((f) => f.revenue), backgroundColor: '#0b1f3a' }],
      },
    ...
    return () => {
      ...
      flightChartRef.current?.destroy();
    };
  }, [data]);
    ...
    <div className="container mt-4">
      ...
      <div className="row g-3 mb-4">
        <div className="col-md-3">
          <div className="card p-3 text-center">
            ...
            <h3 className="text-primary">${data.total_revenue.toFixed(2)}</h3>
          </div>
        </div>
        <div className="col-md-3">
          <div className="card p-3 text-center">
            <span className="text-muted small">Total Expenses</span>
            <h3 className="text-danger">${data.total_expenses.toFixed(2)}</h3>
          </div>
        </div>
        <div className="col-md-3">
          <div className="card p-3 text-center">
            <span className="text-muted small">Net Profit</span>
            <h3 className={data.net_profit >= 0 ? 'text-success' : 'text-danger'}>${data.net_profit.toFixed(2)}</h3>
          </div>
        </div>
        <div className="col-md-3">
          <div className="card p-3 text-center">
      ...
      <div className="row g-4">
        <div className="col-md-7">
          <h5>Revenue vs Expenses by Month</h5>
          <canvas ref={monthlyCanvasRef} />
        </div>
        <div className="col-md-5">
          <h5>Top Flights by Revenue</h5>
