...
export async function createFaultReport({ aircraft_id, reported_by, fault_description, severity }) {
  const [result] = await pool.query(
    "INSERT INTO fault_reports (aircraft_id, reported_by, fault_description, severity, status, reported_at) VALUES (?, ?, ?, ?, 'Reported', NOW())",
    [aircraft_id, reported_by || null, fault_description, severity]
  );
...
export async function getAllFaults() {
  const [rows] = await pool.query(
    `SELECT fr.*, a.tail_number, a.model, u.name AS reported_by_name
     FROM fault_reports fr
     JOIN aircraft a ON fr.aircraft_id = a.id
...
export async function getFaultById(id) {
  const [rows] = await pool.query('SELECT * FROM fault_reports WHERE id = ?', [id]);
  return rows[0] || null;
}
