...
export async function report(req, res) {
  try {
    ...
    const id = await createFaultReport({ aircraft_id, reported_by: user?.id || null, fault_description, severity });
...
export async function list(req, res) { try { res.status(200).json(await getAllFaults()); } catch (error) { console.error(error); res.status(500).json({ error: 'Failed to fetch fault reports' }); } }
