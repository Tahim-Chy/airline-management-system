...
export async function report(req, res) {
  try {
    ...
    if (!passenger_name || !contact_email || !description) {
      return res.status(400).json({ error: 'Name, contact email, and description are required' });
    }
...
export async function updateStatus(req, res) {
  try {
    ...
    if (!VALID_STATUSES.includes(report_status)) {
