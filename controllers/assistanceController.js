...
export async function submit(req, res) {
  try {
    ...
    if (!booking_id || !request_type) return res.status(400).json({ error: 'Booking ID and request type are required' });
    if (!VALID_TYPES.includes(request_type)) return res.status(400).json({ error: `Request type must be one of: ${VALID_TYPES.join(', ')}` });
