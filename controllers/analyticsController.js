...
// Merges revenue-by-month and expense-by-month into one aligned series so a
// single Chart.js chart can plot both bars side by side.
...
export async function getDashboardData(req, res) {
  try {
    const [totalRevenue, totalExpenses, revenueByMonth, expensesByMonth, revenueByFlight, bookingCount] = await Promise.all([
      getTotalRevenue(),
      getTotalExpenses(),
      getRevenueByMonth(),
      getExpensesByMonth(),
    ...
    res.status(200).json({
      ...
      monthly: mergeByMonth(revenueByMonth, expensesByMonth),
      revenue_by_flight: revenueByFlight,
    });
  } catch (error) {
    console.error(error);
