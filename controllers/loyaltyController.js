...
export async function claimPoints(req, res) {
  try {
    ...
    res.status(200).json({ message: `${earnedPoints} points added!`, earned_points: earnedPoints, total_points: account.points, tier: tierForPoints(account.points), email: account.email });
  } catch (error) { console.error(error); res.status(500).json({ error: 'Failed to claim points' }); }
}
export async function lookupAccount(req, res) {
  try {
    ...
    res.status(200).json({ email: account.email, name: account.name, points: account.points, tier: tierForPoints(account.points) });
  } catch (error) { console.error(error); res.status(500).json({ error: 'Failed to look up account' }); }
}
